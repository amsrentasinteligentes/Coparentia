...
PUBLICADO y verificado en coparentia.co con sesión real (chips Isa · Alejo · Todos; tarjeta por hijo).
Vercel se atascó (2 despliegues viejos en Queued/Initializing bloqueaban la cola): el usuario los canceló
y el redeploy b4044be salió Ready. Gates veredicto:landing / veredicto:onboarding / veredicto:paywall sin
cambios (techo documentado en "Problemas conocidos"). Sigue pendiente: validación con Ivonne →
cancelación/reembolso; logo plano azul si llega; el usuario borrará el hijo de prueba "Alejo".
...
nueva orden. Carrusel "La app por dentro" de la landing (2026-09-18): ya no muestra onboarding ni paywall (siguen
oscuros y desentonaban): ahora Inicio, Pagos, Calendario, Expediente y Perfil, capturas reales del
interior claro (public/frame-*.png; frame-onboarding/paywall quedan sin uso). Prueba de
cancelación/reembolso con la cuenta de Ivonne: POSPUESTA por decisión del
...
- **veredicto:landing/onboarding/paywall** (2026-09-25, politica de datos): el gate avisa por mtime
  de app/privacidad/page.tsx — esa pantalla NO es ninguna de las 4 del dinero (es una pagina legal de
  solo lectura), no dispara revision. No se relanza el revisor por esto.
